...
**БПОС - безопасное подключение к отечественным сервисам**.  
![БПОС логика](./img/bpos_logic.png)
Для чего это вообще нужно? Пример: вы решили отдохнуть заграницей (Китай) летом. Вы самозанятый. Вдруг, настало 9 число месяца (день оплаты налогов самозанятым), но вы не можете оплатить налог, т.к. не имеете доступа к ЕГР ФНС. В этом случае пригодится доступ к своему собственному БПОС.
## Подготовка
...
- Блок `[Interface]`
    * Address - чисто настройка внутренней БПОС сети
    * PrivateKey - приватный ключ, который мы создали на прошлом шаге
    * ListenPort - порт, к которому клиент сможет обращаться для доступа к БПОС
...
- Блок `[Peer]` - клиент
    ...
    * AllowedIPs - разрешает доступ клиента к ip 10.0.0.2/32 в подсети БПОС.
...
Тут из интересного только AllowedIPs - это значит, что ВЕСЬ ipv4 трафик будет идти через БПОС. (0.0.0.0/0 = ipv4 трафик)  
